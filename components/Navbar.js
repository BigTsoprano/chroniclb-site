...
useEffect(() => {
  ...
  const navigation = [
      { title: "Resources", path: "javascript:void(0)", isDrapdown: true, navs: dropdownNavs },
      { title: "Menu", path: "javascript:void(0)", isDrapdown: false },
      { title: "Deals", path: "javascript:void(0)", isDrapdown: false },
        ...
        <div className="items-center gap-x-14 px-4 max-w-screen-xl mx-auto md:flex md:px-8">
            <div className="flex items-center justify-between py-3 md:py-5 md:block">
                <a style={{display:'flex', flexDirection:'row', alignItems:'flex-end'}} className='' href="javascript:void(0)">
                    <img
                        src="chronic_leaf.png"
                        width={40}
                        height={40}
                        alt="chroniclb"
                    />
                    <a style={{display:'flex', flexDirection:'column',lineHeight:'10px'}} className=' text-base pl-2 text-white font-semibold'>Chronic <span className='italic pl-4 text-sm text-green-700'>Long Beach</span></a>
                        ...
                        navigation.map((item, idx) => {
                            return (
                                <li key={idx}>
                                    {
                                        item.isDrapdown ? (
                                            <a className="w-full flex items-center justify-between gap-1 text-gray-700 hover:text-green-600"
                                            ...
                                            <a href={item.path} className="block text-gray-700 hover:text-green-700">
                                                {item.title}
                                            </a>
                                        )
                                    }
                                    {
                                        item.isDrapdown && drapdownState.idx == idx && drapdownState.isActive ? (
                                            <div className="mt-6 z-20 bg-white inset-x-0 top-20 w-full md:absolute md:border-y md:shadow-md md:mt-0">
                                                                ...
                                                                {dropdownItem.navs.map((navItem, idx) => (
                                                                    <li key={idx} className="group">
                                                                        <a href={navItem.path} className='flex gap-3 items-center'>
                                                                            <div className='w-12 h-12 rounded-full bg-green-700 text-black flex items-center justify-center duration-150 group-hover:bg-green-200 group-hover:text-white md:w-14 md:h-14'>
                                                                                {navItem.icon}
                                                                            </div>
                                                                            <div>
